...
class VK:
    ...
    def get_user_pics(self):
        ...
        response = requests.get(self.BASE_URL + method, params)
        print('У пользователя {} {} {} фото'.format(self.first_name, self.last_name,
...
class GoogleUploader:
    ...
    def upload(self, path):
        ...
        file_list = self.drive.ListFile(
            {'q': "'{}' in parents and trashed=false".format(self.file_dict[self.foldername])}).GetList()
        titles = []
        ...
        for filename in tqdm(os.listdir(path), ncols=80, ascii=True, desc='Загрузка изображений'):
            ...
            if not filename in titles:
                my_file = self.drive.CreateFile(
                    {'title': f'{filename}', 'parents': [{'id': self.file_dict[self.foldername]}]})
